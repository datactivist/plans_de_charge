...
```{r echo=FALSE}
# All icons here : https://ionicons.com/v2/cheatsheet.html or https://fontawesome.com/icons?d=gallery&p=2 
en_cours <- sarah |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
n1 <- en_cours |> distinct(Projet) |> na.omit() |> nrow()
valueBox(n1, color = "#CC0033", caption = ifelse(n1 > 1, "Missions", "Mission"))
...
```{r echo=FALSE}
annee <- sarah |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
n5 <- annee |> distinct(Projet) |> na.omit() |> nrow()
valueBox(n5, color = "#CC0033", caption = ifelse(n5 > 1, "Missions", "Mission"))
```
...
# Tableau des projets
table_projets <- en_cours |> as.data.frame() |> 
...
```{r}
clem <- data |> filter(`Assigné à` == "Clément Mandron")
```
...
```{r echo=FALSE}
# All icons here : https://ionicons.com/v2/cheatsheet.html or https://fontawesome.com/icons?d=gallery&p=2 
en_cours <- clem |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
n1 <- en_cours |> distinct(Projet) |> na.omit() |> nrow()
valueBox(n1, color = "#CC0033", caption = ifelse(n1 > 1, "Missions", "Mission"))
...
```{r echo=FALSE}
annee <- clem |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
n5 <- annee |> distinct(Projet) |> na.omit() |> nrow()
valueBox(n5, color = "#CC0033", caption = ifelse(n5 > 1, "Missions", "Mission"))
```
...
```{r}
guillaume <- data |> filter(`Assigné à` == "Guillaume Martin")
```
...
```{r echo=FALSE}
# All icons here : https://ionicons.com/v2/cheatsheet.html or https://fontawesome.com/icons?d=gallery&p=2 
en_cours <- guillaume |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
n1 <- en_cours |> distinct(Projet) |> na.omit() |> nrow()
valueBox(n1, color = "#CC0033", caption = ifelse(n1 > 1, "Missions", "Mission"))
...
```{r echo=FALSE}
annee <- guillaume |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
n5 <- annee |> distinct(Projet) |> na.omit() |> nrow()
valueBox(n5, color = "#CC0033", caption = ifelse(n5 > 1, "Missions", "Mission"))
```
...
```{r}
# Tableau des heures restantes divisées par semaine jusqu'à fin tache / sous-tache
table_restant_semaine <- guillaume |> 
  filter(`Heures restantes` > 0) |> 
  mutate(today = Sys.Date(),
         nb_week_left = round(as.numeric(difftime(fin, today, units = "weeks")), 0),
         hours_week_left = `Heures restantes` / nb_week_left) |> 
