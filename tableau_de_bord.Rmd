...
en_cours <- data |> filter((any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), .by = Projet)
...
```{r echo=FALSE}
# All icons here : https://ionicons.com/v2/cheatsheet.html or https://fontawesome.com/icons?d=gallery&p=2 
en_cours <- sarah |> filter((any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), .by = Projet)
n1 <- en_cours |> distinct(Projet) |> na.omit() |> nrow()
valueBox(n1, color = "#CC0033", caption = ifelse(n1 > 1, "Missions", "Mission"))
```
