...
# Données
#old_data <- read_csv("project.task.csv")
data <- read_csv("https://odoo.datactivist.coop/datactivist_report/static/src/Taches_-_Export_pour_dataviz.csv") |> 
  rename(Projet = `Projet...1`,
         debut_projet = `Projet...2`,
         fin_projet = `Projet...3`,
         debut = `Date de début`,
         fin = `Date de fin`,
  ...
  mutate(type = case_when(!is.na(tache_parente) ~ "Sous-tâche", .default = "Tâche"),
         ...
         Progression = `Heures passées` / `Heures prévues` *100,
         Progression = as.numeric(str_replace_all(Progression, c("Inf" = "0", "NaN" = "0"))),
         Progression = replace(Progression, which(Progression > 100), 100)) |> 
  ...
  ungroup() |> 
  filter(`Assigné à` %in% c("Allyson Pallisser","Anne-Laure Donzel","Anthony Gigerich","Arthur Sarazin","Candice Fillaud","Clément Feyt","Clément Mandron","Diane Thierry","Emeline Vandeven","Etienne Ferrandi","Etienne Pichot Damon","François Lacombe","Guillaume Martin","Ho-Pun-Cheung Élise","Joël Gombin","Maëlle Fouquenet","Magalie Dartus","Pauline Breton-Chauvet","Samuel Goeta","Sarah Bourgouin","Stéphanie Trichard"))
```


Vue d'ensemble {data-icon="fa-search"}
=====================================  
...
Par projet {data-icon="fa-list-check"}
=====================================  



Allyson {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Allyson Pallisser")
...
box2 <- function(data){
  ...
  n2_factures <- data |> 
    ...
    summarise(round(sum(`Heures prévues`, na.rm = T)/7, 0))
  valueBox(n2, color = "#FF9933", 
           caption = ifelse(abs(n2) > 1, paste("Jours prévus dont", n2_factures, "facturables"), paste("Jour prévu dont", n2_factures, "facturable"))) 
}
box2(en_cours)
```
...
box3 <- function(data){
  ...
  n3_factures <- data |> 
    ...
    summarise(round(sum(`Heures passées`, na.rm = T)/7, 0))
  valueBox(n3, color = "#FFCC00", 
           caption = ifelse(abs(n3) > 1, paste("Jours réalisés dont", n3_factures, "facturables"), paste("Jour réalisé dont", n3_factures, "facturable")))
}
box3(en_cours)
```
...
box4 <- function(data){
  ...
  n4_factures <- data |> 
    ...
    summarise(round(sum(`Heures restantes`, na.rm = T)/7, 0))
  valueBox(n4, color = "#339900", 
           caption = ifelse(abs(n4) > 1, paste("Jours restants dont", n4_factures, "facturables"), paste("Jour restant dont", n4_factures, "facturable")))
}
box4(en_cours)
```
...
box6 <- function(data){
  ...
  n6_factures <- data |> 
    ...
    summarise(round(sum(`Heures prévues`, na.rm = T)/7, 0))
  valueBox(n6, color = "#FF9933", 
           caption = ifelse(abs(n6) > 1, paste("Jours prévus dont", n6_factures, "facturables"), paste("Jour prévu dont", n6_factures, "facturable"))) 
}
box6(annee)
```
...
box7 <- function(data){
  ...
  n7_factures <- data |> 
    ...
    summarise(round(sum(`Heures passées`, na.rm = T)/7, 0))
  valueBox(n7, color = "#FFCC00", 
           caption = ifelse(abs(n7) > 1, paste("Jours réalisés dont", n7_factures, "facturables"), paste("Jour réalisé dont", n7_factures, "facturable")))
}
box7(annee)
```
...
box8 <- function(data){
  ...
  n8_factures <- data |> 
    ...
    summarise(round(sum(`Heures restantes`, na.rm = T)/7, 0))
  valueBox(n8, color = "#339900", 
           caption = ifelse(abs(n8) > 1, paste("Jours restants dont", n8_factures, "facturables"), paste("Jour restant dont", n8_factures, "facturable")))
}
box8(annee)
```
...
tableau_global <- function(data){
  # Barplot dans gt table
  bar_chart <- function(label, height = "16px", fill = "green", background = "white") {
  ...
  table_projets <- data |> 
    ...
    rename(`heures prévues` = `Heures prévues`,
           ...
           `assigné` = `Assigné à`) |> 
    mutate(debut = format(as.POSIXct(debut, format="%Y-%d-%m %H:%M:%S"),"%d %b %Y"),
           fin = format(as.POSIXct(fin, format="%Y-%d-%m %H:%M:%S"),"%d %b %Y")) |> 
    select(Projet, ` `, type, todo, debut, fin, `heures prévues`:`heures restantes`, etape, Progression, état, `assigné`, id) |> 
    mutate_all(as.character) |> 
    mutate_at(vars(everything()), replace_na, replace = "") |> 
    mutate(`heures restantes` = as.numeric(`heures restantes`),
           Progression = purrr::map(Progression, ~bar_chart(label = .x, fill = "#51c230", background = "#da4729")))
  ...
  table_projets |> 
    ...
    gt_fa_column(column = ` `, direction = 1, 
                 palette = if (all(table_projets$` ` == "clipboard-check")) "black" 
                 else if (all(table_projets$` ` == "list-check")) "#e69f00" else c("circle" = "black", "list-check" = "#e69f00")) |>
    ...
    tab_style(  #en gras
      style = list(cell_text(color = "#f44336", align = "center")),
      locations = cells_body(
      columns = `heures restantes`,
      rows = `heures restantes` < 0)) |>
    tab_style(  #en gras
      style = list(cell_text(color = "#38761d", align = "center")),
      locations = cells_body(
      columns = `heures restantes`,
      rows = `heures restantes` >= 0)) |>
    tab_style(  #en gras
    ...
    tab_style(  #en gras
      ...
      locations = cells_body(
      columns = état)) |> 
    cols_align(align = "left", columns = c(Progression)) |>
...
tryCatch(heures_semaine(salarie), error = function(e) NULL)
```
...
tryCatch(heures_mois(salarie), error = function(e) NULL)
```
...
Anne-Laure {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Anne-Laure Donzel")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Anthony {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Anthony Gigerich")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Arthur {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Arthur Sarazin")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Candice {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Candice Fillaud")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Clément.F {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Clément Feyt")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Clément.M {data-navmenu="Par salarié"}
=====================================  
...
Diane {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Diane Thierry")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Élise {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Ho-Pun-Cheung Élise")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Emeline {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Emeline Vandeven")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Etienne.F {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Etienne Ferrandi")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Etienne.P {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Etienne Pichot Damon")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
François {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "François Lacombe")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Guillaume {data-navmenu="Par salarié"}
=====================================  
...
Joël {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Joël Gombin")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Magalie {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Magalie Dartus")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Maëlle {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Maëlle Fouquenet")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Pauline {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Pauline Breton-Chauvet")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Samuel {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Samuel Goëta")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
...
Sarah {data-navmenu="Par salarié"}
=====================================  
...
Stéphanie {data-navmenu="Par salarié"}
=====================================  

```{r}
salarie <- data |> filter(`Assigné à` == "Stéphanie Trichard")
annee <- salarie |> filter(any(debut >= "2023-01-01") | any(debut_projet >= "2023-01-01"), .by = Projet)
en_cours <- salarie |> filter(etat_projet != "Terminé", etat_projet != "Terminé mais pas facturé",
                            (any(debut_projet <= Sys.Date()) & any(fin_projet >= Sys.Date())) | (any(debut <= Sys.Date()) & any(fin >= Sys.Date())), 
                            .by = Projet)
